...
std::string wDataHandler::handleWsMessage(std::string payload){
    std::cout << "PAYLOAD RECEIVED\n";
    if(payload.find("auth") != std::string::npos){
        return("Authorization request accepted.\n");
    }
    else if(payload.find("treat_ack") != std::string::npos){
        return("Treat acknowledges, relaying msg to device\n");
    }
    else if(payload.find("treat_nack") != std::string::npos){
    ...
    if(restinio::http_connection_header_t::upgrade == req->header().connection())
    {
        ...
        [this](auto wsh, auto m){
            if(rws::opcode_t::text_frame == m->opcode() ||
                ...
                {
                    std::string response = handleWsMessage(m->payload());
...
//auto wDataHandler::on_receive_ws(const restinio::request_handle_t& req, rr:route_params_t params);
